...
async def register_driver(driver_data: DriverCreate) -> UserResponse:
    ...
    user =  await user_repo.create(
        driver_data.email,
        get_password_hash(driver_data.password),
        'driver')
    ...
    await driver_repo.create(
        ...
        driver_data.phone,
        driver_data.transport_type)
  
    return UserResponse(id=user.id, email=user.email, role=user.role)


async def register_company(company_data: CompanyCreate) -> UserResponse:
    ...
    user = await user_repo.create(
        company_data.email,
        get_password_hash(company_data.password),
        "company")
