...
async def process_email(
    ...
    if not user:
        await message.answer("Пользователь с таким Email не найден в системе")
        return

    await user_service.send_login_code(email)
    await state.update_data(email=email)
    await state.set_state(AuthState.waiting_for_code)
    await message.answer("Код подтверждения отправлен на вашу почту. Введите его:")
...
async def process_code(
    ...
    if success:
        await state.clear()
        await message.answer("Вход выполнен успешно!", reply_markup=get_main_reply_kb())
    else:
        await message.answer(
            "Невверный код или истек срок его действия истек. Поробуйте еще раз:"
        )
